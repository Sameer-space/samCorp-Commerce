...
const discountController = {
    getAllDiscounts: async (req, res) => {
        try {
            ...
            res.status(200).json({ discounts: formattedDiscounts });
        } catch (error) {
            console.error(error);
    ...
    createDiscount: async (req, res) => {
        try {
            ...
            const { code, description, value, type, startDate, endDate, usability } = req.body;
            ...
            res.status(201).json({ message: 'Discount created successfully', discount: formattedDiscount });
        } catch (error) {
            console.error(error);
            res.status(500).json({ error: 'Internal server error' });
        }
