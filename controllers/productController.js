...
const ProductController = {
    ...
    async createProduct(req, res) {
      try {
        const { name, description, images, category, variants, isActive } = req.body;
        ...
        res.status(201).json({ message: 'Product created successfully', product: modifiedProduct });
      } catch (error) {
        console.error(error);
    ...
    async getAllProducts(req, res) {
      try {
        const products = await Product.find({}, { __v: 0 }); // Exclude __v field
    ...
    async updateProductById(req, res) {
      try {
        ...
        res.json({ message: 'Product updated successfully', product: modifiedProduct });
      } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Internal server error' });
      }
    ...
    async deleteProductById(req, res) {
      try {
        ...
        res.json({ message: 'Product deleted successfully', product: modifiedProduct });
      } catch (error) {
        console.error(error);
        res.status(500).json({ error: 'Internal server error' });
      }
